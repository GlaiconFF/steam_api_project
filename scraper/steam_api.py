...
def search_game(game_id):
    game_reviews_response = session.get(id_reviews+f"{game_id}?json=1&filter=all&language=all&l=en")
    ...
    game_details_response = session.get(item_details+f"?appids={game_id}&cc=en&l=en")
